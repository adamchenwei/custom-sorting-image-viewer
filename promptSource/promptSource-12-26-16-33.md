...
export default function ResultsPage() {
  ...
  const handleMove = async (image: ImageData, targetPath: string) => {
    try {
      ...
      // Move the image and get the new data
...
import path from 'path';
import { getDay } from 'date-fns';
import { processImagesDirectory } from '../../../../scripts/processImages';
...
function applySortOptions(data: any[], sortOptions: MoveRequest['sortOptions']) {
...
export async function POST(request: Request) {
  try {
    ...
    const { items: imageData } = processImagesDirectory(imagesDir, publicDir);
    ...
    return NextResponse.json({ 
      success: true,
      message: 'Image moved successfully and data.json regenerated',
      newPath: targetPath,
      newData: sortedData // Return the sorted data
    });
  } catch (error: any) {
